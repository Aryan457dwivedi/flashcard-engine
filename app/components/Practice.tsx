...
function attachCometTilt(wrap: HTMLElement, glare: HTMLElement, ROTATE = 15, TRANSLATE = 16) {
...
export default function Practice({
  ...
  useEffect(() => {
    ...
    return attachCometTilt(cometRef.current, glareRef.current);
  });
  ...
  const answer = (quality: number) => {
    ...
    setTimeout(() => {
      ...
      setCards(prev => { const next = [...prev]; next[indexSnapshot] = updatedCard; return next; });
    ...
    return (
      <>
        ...
        <div className="done-wrap">
          ...
          <p className="done-sub">You reviewed all <strong>{cards.length}</strong> cards this session.</p>
          <div className="score-ring-wrap">
            <svg width="116" height="116" viewBox="0 0 116 116">
              ...
              />
              <text x="58" y="54" textAnchor="middle" fontFamily="'Fraunces',Georgia,serif"
                fontWeight="700" fontSize="24" fill="#1a1a2e">{score}%</text>
        ...
        <div className="stack-wrap">
          <div className="comet-wrap" ref={cometRef}>
            <div
              ...
              tabIndex={0}
              onKeyDown={e=>{ if(e.code==='Enter'){e.preventDefault();setFlipped(f=>!f);} }}
              role="button"
...
.mastery-card {
  ...
  display:flex; flex-direction:column; align-items:center; gap:5px; border:1px solid;
...
.mastery-val { font-family:'Fraunces',Georgia,serif; font-size:2.2rem; font-weight:700; line-height:1; }
...
.btn-primary:hover { background:rgba(83,74,183,0.90); transform:translateY(-2px); box-shadow:0 6px 28px rgba(127,119,221,0.38); }
...
.btn-ghost:hover { background:rgba(238,237,254,0.75); transform:translateY(-2px); box-shadow:0 4px 18px rgba(127,119,221,0.14); }
